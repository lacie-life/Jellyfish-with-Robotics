...
// Change matrix (for each T265)

Mat K1 = (Mat_<double>(3,3) << 284.0513916015625, 0.0, 427.4079895019531,
                              0.0, 285.19189453125, 401.1722106933594, 
                              0.0, 0.0, 1.0);

Mat K2 = (Mat_<double>(3,3) << 284.285400390625, 0.0, 431.7026062011719,
                              0.0, 285.2674865722656, 385.2261962890625, 
                              0.0, 0.0, 1.0);

Mat R = (Mat_<double>(3,3) << 9.99994218e-01, -8.26635107e-04, -3.29655036e-03, 
                              8.28813005e-04,  9.99999523e-01,  6.59199432e-04,
                              3.29600368e-03, -6.61927043e-04,  9.99994397e-01);

Mat D1 = (Mat_<double>(1,4) << -0.0015451919753104448, 0.04006154090166092, -0.03794296830892563, 0.006493294145911932);

Mat D2 = (Mat_<double>(1,4) << 0.0007516997284255922, 0.032092008739709854, -0.03002322092652321, 0.004072860814630985);


Mat Q, P1, P2;
Mat R1, R2;
Vec3d T(-6.42428249e-02, -1.58874827e-05, 3.78768891e-06);
Vec2d size_input(848, 800);
Vec2d size_output(800,800);
...
//  FileStorage param_file = FileStorage(name, FileStorage::READ);
