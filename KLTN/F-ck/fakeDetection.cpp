...
void fake_point(float x, float y, float z){
    srand (time(NULL));
    fake_x_ = fake_x + (rand() % 5)/100 + fake_x*abs(x/4);

    srand (time(NULL));
    fake_y_ = fake_y + (rand() % 5)/100 + fake_y*abs(y/4);

    srand (time(NULL));
    fake_z_ = abs(z)/100 + (rand() % 5 + 1);

//    std::cout << "Marker position x: " << fake_x_ << " y: " << fake_y_ << " z: " << fake_z_ << std::endl;
    std::cout << "Marker position x: " << fake_x_ << " y: " << fake_y_ << std::endl;
}

void quternionsToRPY(rs2_pose pose_data){
    float x, y, z, w;
  ...
  if(side == CameraSide::LEFT)
  {
    ...
    if (ids.size() > 0)
    {
        ...
        //  cout << " Marker ID: " << ids[i] << "\t";
          ...
          fake_point(tvecs[i](0), tvecs[i](1), tvecs[i](2));
