...
#include "../../include/my_manipulator/my_manipulator.h"
#include "../../include/manipulator_base/manipulator.h"
#include <iostream>

superManipulator::superManipulator()
{}
superManipulator::~superManipulator()
{
    delete kinematics_;
...
void superManipulator::initSuperManipulator(bool using_actual_robot_state, STRING usb_port, STRING baud_rate, float control_loop_time)
{
    /*****************************************************************************
...
//  kinematics_ = new kinematics::SolverUsingCRAndSRPositionOnlyJacobian();
    ...
    if(using_actual_robot_state)
    {
        ...
        *****************************************************************************/
        ...
        actuator_ = new dynamixelManipulator::JointDynamixelControl(control_loop_time);
        ...
        *****************************************************************************/
//         tool_ = new dynamixelManipulator::GripperDynamixel();
//
...
void superManipulator::processSuperManipulator(double present_time)
{
    JointWayPoint goal_joint_value = getJointGoalValueFromTrajectory(present_time);
    for ( int8_t index = 0; index < goal_joint_value.size(); index++)
    {
        std::cout << goal_joint_value.at(index).position << std::endl;
    }
