...
#include "../../include/manipulator_base/manipulator_common.h"
#include <iostream>
...
Pose Manipulator::getComponentPoseFromWorld(Name component_name)
{
    ...
    std::cout << component_.at(component_name).pose_from_world.kinematic.position << std::endl;
    //std::cout << component_name << std::endl;
...
Name Manipulator::findComponentNameUsingId(int8_t id)
{
    ...
    for (it_component = component_.begin(); it_component != component_.end(); it_component++)
    {
        ...
        //std::cout << unsigned(component_.at(it_component->first).joint_constant.id) << std::endl;
